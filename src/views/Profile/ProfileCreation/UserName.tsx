...
const minimumCakeToRegister = new BigNumber(REGISTER_COST).multipliedBy(
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        if (response.ok) {
          const dateCreated = formatDistance(
    ...
    <>
      ...
      {!hasMinimumCakeRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(
            1098,
            `A minimum of ${REGISTER_COST} SHIFT is required`,
            { num: REGISTER_COST }
          )}
        </Text>
      )}
