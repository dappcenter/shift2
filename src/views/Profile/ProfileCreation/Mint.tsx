...
const minimumCakeBalanceToMint = new BigNumber(MINT_COST).multipliedBy(
...
const Mint: React.FC = () => {
  ...
  const {
    ...
    onRequiresApproval: async () => {
      ...
      try {
        const response = await cakeContract.methods
          .allowance(account, bunnyFactoryContract.options.address)
          .call();
    ...
    onConfirm: () => {
      return bunnyFactoryContract.methods
        .mintNFT(bunnyId)
        .send({ from: account });
    },
  ...
  return (
    <>
      ...
      <Text as="p">
        {TranslateString(
          786,
          "Every profile starts by making a “starter” collectible (NFT)."
        )}
      </Text>
      <Text as="p">
        {TranslateString(
          788,
          "This starter will also become your first profile picture."
        )}
      </Text>
      <Text as="p" mb="24px">
        {TranslateString(
      ...
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {TranslateString(999, `Cost: ${MINT_COST} SHIFT`, {
              num: MINT_COST,
            })}
          </Text>
          {nfts.map((nft) => {
            const handleChange = (value: string) =>
              setBunnyId(parseInt(value, 10));
          ...
          {!hasMinimumCakeRequired && (
            <Text color="failure" mb="16px">
              {TranslateString(
                1098,
                `A minimum of ${MINT_COST} SHIFT is required`
              )}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={
              bunnyId === null || isConfirmed || isConfirming || isApproved
            }
            isApproving={isApproving}
            isConfirmDisabled={
              !isApproved || isConfirmed || !hasMinimumCakeRequired
            }
